...
def create_rect(h, w, starting_point=np.array([0., 0., 1.]), h_vec=np.array([1., 0., 0.]), w_vec=np.array([0., 1., 0.])):
    return [starting_point + i / (w - 1) * w_vec + j / (h - 1) * h_vec for i in range(w) for j in range(h)], []


def create_rect_x(h, w, starting_point=np.array([0., 0., 1.]), h_vec=np.array([1., 0., 0.]), w_vec=np.array([0., 1., 0.])):
