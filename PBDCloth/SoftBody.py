...
class SoftBody:
    ...
    def _solve_constrains(self):
        for _ in range(self.iteration_count):
            ...
            for c in self.constrains:
                c.iterate_constrain(self.point_position, self.point_old_position)
    ...
    def _iterate_constrains_connections(self):
        for index in range(self.connection_amount):
            ...
            direction = self.point_position[i] - self.point_position[j]
            if np.linalg.norm(direction):
                ...
                if self.point_is_free[i]:
                    self.point_position[i] -= offset / 2
                if self.point_is_free[j]:
                    self.point_position[j] += offset / 2
    ...
    def calculate_next_positions(self, dt):
        ...
        self.point_position += self.point_position - self.point_old_position + acc * dt * dt * self.point_is_free[
            ..., None]
